...
# 1. целочисленный тензор со случайным значением
x = torch.randint(low=1, high=10, size=(1,), dtype=torch.int32)
print("x int:", x)

# 2. преобразуем к float32 и включаем градиент
x = x.to(torch.float32).requires_grad_()
print("x float:", x)

# номер по списку группы в ЭИОС
num = 11  # здесь поставь свой номер
...
y = x ** n                    # возведение в степень n
y = y * torch.randint(1, 11, (1,), dtype=torch.float32)  # умножение на случайное 1..10
y = torch.exp(y)              # экспонента

# 4. значение производной по x
y.backward()
print("y:", y.item())
